...
;(function ($, window, document) {
    ...
    function Indicator(container, indOption) {
        ...
        console.log("Indicator合并后输出:")
		console.log(indOption)
        this.indOption = indOption;

        //如果选择器字符串在容器中传递，则创建一个查询选择器
        ...
        this.canElm = canElm; // dom对象将要在哪里绘制图形

        this.ctx = canElm.getContext('2d'); //2d的canvas画布
    ...
    Indicator.prototype = {
        ...
        init: function () {
            var indOption = this.indOption,
                canElm = this.canElm,
                ctx = this.ctx,
                dim = (indOption.radius + indOption.barWidth) * 2, //圆环直径
                center = dim / 2; //居中位置的x y坐标    直径/2
            ...
            //画边框
            ctx.strokeStyle = indOption.barBgColor; //背景颜色
            ctx.lineWidth = indOption.barWidth + indOption.borderBarWidth*2;
            ctx.beginPath();
            ctx.arc(center, center, indOption.radius, 0, 2 * Math.PI);
            ctx.strokeStyle = indOption.borderColor;
            ctx.stroke();
            //画一个灰色的圆
            ctx.strokeStyle = indOption.barBgColor; //背景颜色
    ...
    radialIndicator.defaults = {
        ...
        barWidth: 6, //bar width
        borderBarWidth:1,// border width
